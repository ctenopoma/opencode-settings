...
**UX（業務に乗せるための工夫）**:
- **勝手に再描画しない**: 画面は自動で書き換えない。AI が設計書を直す等でファイルが変わると上部に「🔄 更新あり」バナーが出るだけ。反映タイミングは人間が選ぶ（コメント入力中の取りこぼしなし）。
- **要対応だけ見る**: ナビの「要対応のみ」で、承認待ち・差し戻し中・未解決コメント有りだけに絞り込める。各タブに未対応件数バッジ。
- **設計書を読みやすく**: 本文は Markdown としてレンダリング（表・見出し）。目次チップで節へジャンプ。
- **レビューポイントが分かる**: コメントは節見出しに紐づけ（ドロップダウン選択）、コメントの 📍 をクリックすると該当節へスクロール＆ハイライト。
- **再提出の差分**: AI が直して再提出すると「⚠ 前回レビュー後に更新されています」と表示し、前回レビュー時点との差分を開いて確認できる。
